...
class PostgreSQLAdapter(
    ...
    override suspend fun saveAuthEvent(event: AuthEvent, metadata: EventMetadata): String {
        return try {
            val eventId = "evt_${kotlinx.datetime.Clock.System.now().toEpochMilliseconds()}"
    ...
    override suspend fun saveAuthEvent(event: AuthEvent, metadata: EventMetadata): String {
        return try {
            ...
            val eventDoc = mapOf(
                ...
                "createdAt" to kotlinx.datetime.Clock.System.now().toString()
            )
...
object PersistenceExamples {
    ...
    suspend fun usageExample(adapter: PersistenceAdapter) {
        ...
        // Save auth event
        val event = Authentication.SignInCompleted(user, TokenPair("token", "refresh", kotlinx.datetime.Clock.System.now()))
